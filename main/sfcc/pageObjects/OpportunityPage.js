...
class OpportunityPage {
    ...
    constructor(page) {
        ...
        this.AffärsmöjligheterDropdown = this.page.locator("//a[@class='select']").nth(1);
        ...
        //this.AffarsmojlighetskallaPicklistDownIcon = this.page.locator("//button[@aria-label='Affärsmöjlighets källa']//following-sibling::div//lightning-icon[@icon-name='utility:down']//lightning-primitive-icon");
        this.AffarsmojlighetskallaPicklistDownIcon = this.page.locator("//div[contains(@class,'active') and contains(@class,'window')]//button[@aria-label='Affärsmöjlighets källa']//parent::div//../../../..//lightning-base-combobox");
        // this.AffarsmojlighetskallaPicklistDownIcon = this.page.locator("//button[@aria-label='Affärsmöjlighets källa']//parent::div//../../../..//lightning-base-combobox");
        // //div[contains(@class,'active') and contains(@class,'window')]//button[@aria-label='Affärsmöjlighets källa']//parent::div//../../../..//lightning-base-combobox
        ...
        this.antalTextbox = this.page.locator("//label[contains(normalize-space(.),'Antal')]//following-sibling::input").first();
        this.manadsavgiftTextbox = this.page.locator("//label[contains(normalize-space(.),'Månadsavgift')]//following-sibling::input").first();
        this.engangsavgiftTextbox = this.page.locator("//label[contains(normalize-space(.),'Engångsavgift')]//following-sibling::input").first();
        this.avtalstidTextbox = this.page.locator("//label[contains(normalize-space(.),'Avtalstid')]//following-sibling::input").first();
        ...
        //CO2 Address Auto Suggestion List
        //this.CoAddressAutoSuggestionList =this.page.locator("//c-address-validation//table//tbody/tr[1]//input[@class='slds-input']//parent::div//following::div//div//ul");
        this.CoAddressAutoSuggestionList =this.page.locator("//c-address-validation//table//tbody/tr[1]//input[@class='slds-input']//parent::div//following::div//div//ul[@role='presentation']");        
    }
    ...
    async createStandardProcessOpportunity(TestData, utilityFunction) {
        ...
        const accountID = await utilityFunction.RunSOQLQuery("select id from account where Org_Nr__c= '5599900189'");
    ...
    async createAffarsmojlighetOpportunity(TestData, utilityFunction) {
        ...
        await this.page.getByRole('option', { name: 'A3' }).click();        
        //await this.page.locator("//div[contains(normalize-space(.),'Återförsäljare Namn') and contains(@class,'uiInput')]//a[@class='select' and @role='combobox']").click();   
        //await this.page.getByRole('combobox', { name: 'Återförsäljare' }).click();        
        //await this.AffärsmöjligheterDropdown.click();    
        //await this.page.getByRole('option', { name: 'Ingen' }).click();
        await this.saveButton.click();
        await this.page.getByRole('link', { name: 'Konton', exact: true }).click();
        const OpportunityID = await utilityFunction.RunSOQLQuery("select id from opportunity where name= \'" + OpportunityName + "\'");
        await this.page.goto(secretsData.get("environmentURL") + "/lightning/r/Opportunity/" + OpportunityID + "/view");
    ...
    async createProfileRecordTypeBasedOpportunity(TestData, utilityFunction) {
        ...
        if (TestData.get("Profile") == "SME" && TestData.get("OpportunityRecordType") == "Förenklad process" && TestData.get("UsingAccountButton") == "No") {
            ...
            await this.page.locator("//div[contains(@class,'active') and contains(@class,'window')]//button[@aria-label='Affärsmöjlighets källa']").click({force : true});
            ...
            //await this.AffarsmojlighetskallaPicklistDownIcon.click({force: true});           
            

            await this.page.waitForTimeout(3000);
            await this.page.getByRole('option', { name: 'Whisbi' }).hover();
    ...
    async createCRMFiberOpportunity(TestData, utilityFunction) {
        ...
        //const todaysDate = await utilityFunction.TodaysDate();
        const todaysDate = await utilityFunction.TodaysDateDDMMYYYY();
        await this.AvslutsdatumDateField.type(todaysDate);
    ...
    async addContactOnOpportunity(TestData) {
        ...
        await this.searchContactTextbox.fill("Test Narayana Velagala");
        await this.page.waitForTimeout(2000);
        await this.searchContactTextbox.clear();
        //await this.searchContactTextbox.type(TestData.get("ContactName"));
        await this.searchContactTextbox.type("Test Narayana Velagala");
        ...
        await this.page.getByRole('link', { name: 'Test Narayana Velagala' }).click();
    ...
    async performPrisforhandling(TestData) {
        ...
        //..**console.log ("performPrisforhandling -- Products to be added "+Products);
        ...
        for (let iteration = 0; iteration < Products.length; iteration++) {
            ...
            if (Products[iteration] === "Bredband Plus" || Products[iteration] === "Bredband Start" || Products[iteration] === "Bredband Pro" ||
                ...
                console.log("ConnectedOfficeFlag = Yes");            
        ...
        if (ITDaasFlag === "Yes") {
            ...
            await this.page.getByLabel('*Servers (ex.egna lokalt och/eller externt)').fill('Test Automation Servers (egna lokalt och/eller externt)');
            ...
            //await this.page.getByLabel('*Office eller Microsoft 365 (ex. obligatorisk beskrivning)').fill('Test Automation Office eller Microsoft 365');   
            await this.page.getByLabel('*Microsoft 365 eller Office (ex. antal och typ av licenser)').fill('Test Automation Office eller Microsoft 365');            
        ...
        if (ConnectedOfficeFlag === "Yes") {
            ...
            if (TestData.get("COLeveranskontrollType") === "Excel") {
                ...
                await this.page.getByText('Filen har laddats upp',{ exact: true }).click();
                ...
                console.log("Total Address present :: "+ addressListLength);
                await this.page.waitForTimeout(5000);
                await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody//tr[" + addressListLength + "]//select[@name='selectedServicePoint']").hover();

                for (let i = 1; i <= addressListLength; i++) {
                    await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody//tr[" + i + "]//select[@name='selectedServicePoint']").hover();
                ...
                for (let i = 0; i <= Address.length-1; i++) {                    
                    await this.page.locator("//c-address-validation//table//tbody/tr[1]//input[@class='slds-input']").pressSequentially(Address[i],{ delay: 100 } );                   
                    await this.page.locator("//ul[@role='presentation' and @class!='scrollable']//li[1]//div//span//span[contains(normalize-space(.),'" + Address[i] + "')]").click();
                    //ul[@role='presentation' and @class!='scrollable']//li["+ i +"]//div//span//span[contains(normalize-space(.),'Adr Solnav 94, 17190 Solna, Sverige')]
                    //ul[@role='presentation' and @class!='scrollable']//li["+ i +"]//div//span//span[contains(normalize-space(.),'Solana AB, 57730 Hultsfred, Sverige')]
                    //ul[@role='presentation' and @class!='scrollable']//li["+ i +"]//div//span//span[contains(normalize-space(.),'Nanna Svartz väg 1, 17165 Solna, Sverige')]
                    await this.page.waitForTimeout(2000);                   
                ...
                await this.page.locator("//c-address-validation//table//tbody/tr[1]//td//lightning-icon[contains(@class,'delete')]/span/lightning-primitive-icon").click();
                await this.page.getByRole('button', { name: 'Ja' }).click();
            }

            // click checkbox Breadband start
            await this.page.locator("//*[@data-omni-key='CustomLWC1' and @data-omni-layout='lightning']//div['.slds-m-bottom_large']//span[text()='Bredband Start']//following-sibling::span//span['.slds-checkbox_faux']").click();
            await this.page.waitForTimeout(2000);                 
            ...
            await this.page.locator("//*[@data-omni-key='CustomLWC1' and @data-omni-layout='lightning']//tbody[@class='custom-table-body']/..//thead/tr[1]/td[contains(normalize-space(.),'Site')]").click();
            await this.page.locator("//*[@data-omni-key='CustomLWC1' and @data-omni-layout='lightning']//tbody[@class='custom-table-body']/..//thead/tr[1]/td[contains(normalize-space(.),'Bredband Plus')]").click();
            await this.page.locator("//*[@data-omni-key='CustomLWC1' and @data-omni-layout='lightning']//tbody[@class='custom-table-body']/..//thead/tr[1]/td[contains(normalize-space(.),'Bredband Pro')]").click();
            
            //Bredband Start
            await this.page.locator("//*[@data-omni-key='CustomLWC1' and @data-omni-layout='lightning']//tbody[@class='custom-table-body']/..//thead/tr[1]/td[contains(normalize-space(.),'Bredband Start')]").click();
            await this.page.locator("//*[@data-omni-key='CustomLWC1' and @data-omni-layout='lightning']//tbody[@class='custom-table-body']/..//thead/tr[1]/td[contains(normalize-space(.),'Datanet')]").click();

           
            var COFiberText;
            for(let i = 1; i <= TestData.get("CONumberOfAddresses"); i++){

                        // columns = No of Products + 2
                        //--'Bredband Pro'
                            //Fiber - FA_WHOLESALE_FIBER
                            //  Möjligt leveransdatum : 2025-07-04
                            //  SLA C4 Tillgänglig               
                        ...
                        //'Datanet'
                            //Fiber - L3VPN_WHOLESALE_FIBER
                            //Möjligt leveransdatum : 2025-07-04
                            //SLA C4 Tillgänglig

                    for(let j=3 ; j<=6; j++){ 
                        if ( j !== 5){
                            await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td["+ j + "]/div[contains(normalize-space(.),'Fiber - FA_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - FA_FASTPLACNOD') or contains(normalize-space(.),'Fiber - L3VPN_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - L3VPN_FASTPLACNOD')]").click();
                            //await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td["+ j + "]/div[contains(normalize-space(.),'Fiber - FA_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - FA_FASTPLACNOD') or contains(normalize-space(.),'Fiber - L3VPN_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - L3VPN_FASTPLACNOD')]").click();
                            //await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td["+ j + "]/div[contains(normalize-space(.),'Fiber - FA_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - FA_FASTPLACNOD') or contains(normalize-space(.),'Fiber - L3VPN_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - L3VPN_FASTPLACNOD')]").click();
                            const COFiberText = await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td["+ j + "]/div[contains(normalize-space(.),'Fiber - FA_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - FA_FASTPLACNOD') or contains(normalize-space(.),'Fiber - L3VPN_WHOLESALE_FIBER') or contains(normalize-space(.),'Fiber - L3VPN_FASTPLACNOD')]").textContent();
                            //**    // console.log( "COFiberText :: "+ COFiberText + " and Table cell number is [ "+ i + ". "+ j + " ]");                                    
                            if (COFiberText === 'Fiber - FA_WHOLESALE_FIBER' || COFiberText === 'Fiber - L3VPN_WHOLESALE_FIBER') {
                                await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td[j]/div[contains(normalize-space(.),'" + COFiberText + "')]//div[2]//span[contains(normalize-space(.),'Möjligt leveransdatum :')]").click();
                            //**   // console.log( "Table cell number is ["+ i + ", "+ j + "] - COFiberText -  "+ COFiberText);  
                            }
                        }                    
                    }

                //Bredband Pro td[4] and Datanet [6]
                await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td[5]//div[text()='Hera förfrågan krävs']").click();
                await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td[4]/div[contains(normalize-space(.),'SLA C4 Tillgänglig')]").click();
				await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td[6]/div[contains(normalize-space(.),'SLA C4 Tillgänglig')]").click();
                ...
                //await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td[4]/div[contains(normalize-space(.),'SLA C4 Tillgänglig')]").click();
				//await this.page.locator("//*[@data-omni-key='CustomLWC1']//table//tbody/tr[@class='background']["+i+"]/td[5]/div[contains(normalize-space(.),'SLA C4 Tillgänglig')]").click();
        ...
        if (TestData.get("SalesFlowType") === "NewSales"){
            console.log("SalesFlowType == NewSales");
        } else if (TestData.get("SalesFlowType") === "Renegotiation" || TestData.get("SalesFlowType") === "LegacyNegotiation" || TestData.get("SalesFlowType") === "Inforhandling" || TestData.get("SalesFlowType") === "Tillaggsforhandling" ){   
                console.log("SalesFlowType == " + TestData.get("SalesFlowType")); 
                await this.page.waitForTimeout(15000);
                await this.page.locator("//table/tr/td[1]/div").first().hover();
                var KanOmforhandlasInMADAM = await this.page.locator("//table/tr/td[1]/div").first().textContent();
                var ContractInSalesforceAvtal = await this.page.locator("//table/tr/td[2]/div").first().textContent();
                console.log("SalesFlowType == " + TestData.get("SalesFlowType") + ":: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + "' And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + "'.");                          
        } else if (TestData.get("SalesFlowType") === "UtilizerBreakout"){
            await this.page.waitForTimeout(5000);
                console.log("SalesFlowType == UtilizerBreakout");
            if (TestData.get("UtilizerBreakoutContractStatus") === "Draft") {
                console.log ("UtilizerBreakoutContractStatus = Draft");
            }else if (TestData.get("UtilizerBreakoutContractStatus") === "Active") {
                console.log ("UtilizerBreakoutContractStatus = Active");
                await this.page.waitForTimeout(10000);
                var KanOmforhandlasInMADAM = await this.page.locator("//table/tr/td[1]/div").first().textContent();
                var ContractInSalesforceAvtal = await this.page.locator("//table/tr/td[2]/div").first().textContent();
                console.log("SalesFlowType == " + TestData.get("SalesFlowType") + " And UtilizerBreakoutContractStatus =='Active' :: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + "' And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + "'.");                          
            }     
        }
        ...
        if (TestData.get("SalesFlowType") === "NewSales") {
            ...
            console.log ("SalesFlowType = Renegotiation");            

            //Page 0 -: Godkända/Icke godkända avtal att omförhandla
            await this.page.getByRole('heading', { name: 'Godkända/Icke godkända avtal' }).first().hover();
            ...
            //const KanOmforhandlasInMADAM = await this.page.locator("//vlocity_cmt-omniscript-step[@data-omni-key='MessageNumber6']//table/tr/td[1]/div").textContent();
            //const ContractInSalesforceAvtal = await this.page.locator("//vlocity_cmt-omniscript-step[@data-omni-key='MessageNumber6']//table/tr/td[2]/div").textContent();           
            await this.page.getByText('Y', { exact: true }).nth(1).click();
            await this.page.getByRole('button', { name: 'Nästa' }).click();

            if (KanOmforhandlasInMADAM.trim()==='Y' && ContractInSalesforceAvtal.trim()==='Y'){
                
                //Page 1- Kan Omförhandlas?   --Can it be renegotiated?
                await this.page.getByRole('heading', { name: 'Kan omförhandlas?'}).first().hover(); 
                ...
                await this.page.getByRole('button', { name: 'Nästa' }).click();
                // Page 2- Du har redan ett existerande ramavtal i Salesforce. Välj nedan om du vill Omförhandla, Tilläggsförhandla eller Införhandla
                await this.page.getByRole('heading', { name: 'Du har redan ett existerande ramavtal i Salesforce. Välj nedan om du vill Omförhandla, Tilläggsförhandla eller Införhandla'}).click();            
                await this.page.getByText('Omförhandling : Aktivt avtal som skall omförhandlas med möjlighet att lägga till nya produkter/tjänster.').hover();
                await this.page.getByText('Införhandling : Komplettering av abonnemangsformer som ej fanns med vid avtalstecknande.').hover();
                await this.page.getByText('Tilläggsförhandling : Enbart tillägg av nya produkter/tjänster i aktivt avtal. (Det går ej att omförhandla befintliga produkter/tjänster).').hover();
                await this.page.getByRole('button', { name: 'Nästa' }).click(); 
            }else if (KanOmforhandlasInMADAM.trim()==='Y' && ContractInSalesforceAvtal.trim()==='N') {
                
                console.log("SalesFlowType == Renegotiation :: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + " And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + " '.");                          

                    // Select agreement to renegotiate
                    await this.page.getByRole('heading', { name: 'Välj avtal att omförhandla'}).first().hover(); 
                    await this.page.getByRole('heading', { name: 'Välj avtal att omförhandla'}).click();
                    //Please select an agreement to be renegotiated
                    await this.page.getByText('Vänligen välj ett avtal som ska omförhandlas.').click();
                    await this.page.locator("//lightning-primitive-input-checkbox[@variant='standard']").first().click();
                    await this.page.waitForTimeout(2000);
                    //..**input [@type='checkbox']
                    //await this.page.locator("//lightning-primitive-input-checkbox[@variant='standard']//input[@type='checkbox']").first().click();
                    await this.page.locator("//lightning-primitive-input-checkbox[@variant='standard']").first().click();
                    await this.page.waitForTimeout(2000);
                    //..**Choose what to do            
                    await this.page.getByText('Välj vad du ska göra').click();
                    //..**Additional agreement - Addition of new product family, no approval needed (work mobile is not negotiable)
                    await this.page.getByText('Tilläggsavtal - Tillägg av ny produktfamilj, inget godkännande behövs (jobbmobil går ej att prisförhandla)').hover();
                    //..**Renegotiation - This is an early renegotiation, needs to be approved by your immediate manager
                    await this.page.getByText('Omförhandling - Detta är en förtidig omförhandling, behöver godkännas av närmsta chef').hover();
                    await this.page.getByText('Omförhandling - Detta är en förtidig omförhandling, behöver godkännas av närmsta chef').click();
                    await this.page.getByRole('button', { name: 'Nästa' }).click();
                    //Verify/Check Agreement
                    await this.page.getByRole('heading', { name: 'Verifiera/Kontrollera Avtal'}).click();
                    //This agreement will be renegotiated, please verify and click "next"
                    await this.page.getByText('Detta avtal kommer att omförhandla, vänligen verifiera och klicka "next"').click();
                    //Please note that this agreement has a length
                    await this.page.getByText('Vänligen notera att detta avtal har en längd').click();
                    await this.page.getByRole('button', { name: 'Nästa' }).click();
                
            }else{
                console.log("Condition not Handled for SalesFlowType == Renegotiation :: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + "' And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + "'.");
            }
        ...
        /*
            ...
            */
            ...
            //await this.page.getByText('Omförhandling : Aktivt avtal som skall omförhandlas med möjlighet att lägga till nya produkter/tjänster.', { exact: true }).click(); 
            //await this.page.getByRole('button', { name: 'Nästa' }).click();
            
            //**.. */
           
            //Page3 -Skicka mandatförfrågan?  //Send mandate request?
            await this.page.getByRole('heading', { name: 'Skicka mandatförfrågan?' }).first().hover();            
            await this.page.getByRole('heading', { name: 'Skicka mandatförfrågan?' }).click();
            //This offer needs approval
            await this.page.getByText('Detta erbjudande behöver godkännande').click();
            await this.page.getByLabel('KOMMENTARER').click();
            await this.page.getByLabel('KOMMENTARER').fill('Test Automation');
            await this.page.getByRole('button', { name: 'Nästa' }).click();

            //Page 4-- Mandate Request sent
            await this.page.getByRole('heading', { name: 'Mandatförfrågan skickad' }).click();            
            //Approval request sent to Sales Manager/Sales Director
            await this.page.getByText('Godkännandebegäran skickas till Försäljningschef/Försäljningsdirektören').click();            
            await this.page.getByRole('button', { name: 'Nästa' }).click();

        } else if (TestData.get("SalesFlowType") === "LegacyNegotiation") {

            console.log ("SalesFlowType = LegacyNegotiation");

            var KanOmforhandlasInMADAM ;
            var ContractInSalesforceAvtal ;
            var KanOmforhandlasInMADAM = await this.page.locator("//table/tr/td[1]/div").first().textContent();
            var ContractInSalesforceAvtal = await this.page.locator("//table/tr/td[2]/div").first().textContent();

            console.log("KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + " And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + " '.");  

            if ( KanOmforhandlasInMADAM.trim() ==='Y' && ContractInSalesforceAvtal.trim() ==='N'){
                console.log("SalesFlowType == LegacyNegotiation :: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + " And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + " '.");                          
                await this.page.getByRole('heading', { name: 'Godkända/Icke godkända avtal' }).click();
                if (TestData.get("LegacyNegotiation") === "LegacyOmforhandlingOE"){
                    await this.page.locator("//table/thead/tr/th[normalize-space()='Godkänd växellösning att omförhandla i SF']//..//..//../tr/td[11]/div[contains(text(),'Y')]").click();
                }
                await this.page.getByText('Vissa avtal kan omförhandlas').click();
                await this.page.getByRole('button', { name: 'Nästa' }).click();


                //Page Välj avtal att omförhandla -- Select agreement to renegotiate
                await this.page.getByRole('heading', { name: 'Välj avtal att omförhandla' }).click();                
                await this.page.getByText('Vänligen välj ett avtal som ska omförhandlas.').click();
                await this.page.locator("//lightning-primitive-input-checkbox[@variant='standard']").first().click();
                await this.page.waitForTimeout(2000);
                ...
                //if (TestData.get("LegacyNegotiation") === "LegacyOmforhandlingOE"){
                //      await this.page.locator("//p[contains(text(),' Avtalet innehåller Office Extension. Om avtalet omförhandlas med mobilt/växel kommer OE anknytningar endast gå att teckna tillfälligt i väntan på migrering efter omförhandlingen. Fortsätt endast om kunden ska migrera till en ny växellösning.')]").click();
                //      await this.page.locator("//lightning-primitive-input-checkbox[@variant='standard']").first().click();
                //}
                //      await this.page.getByRole('button', { name: 'Nästa' }).click();               
                //      await this.page.locator("//lightning-primitive-input-checkbox[@variant='standard']").first().click();LegacyOmforhandling
                
                if ((TestData.get("LegacyNegotiation") === "LegacyOmforhandlingOE") ||  (TestData.get("LegacyNegotiation") === "LegacyOmforhandling")) {
                    await this.page.getByText('Omförhandling - Detta är en förtidig omförhandling, behöver godkännas av närmsta chef').click();
                } else if (TestData.get("LegacyNegotiation") === "LegacyTillagsforhandling") {
                ...
                await this.page.getByText('Felmeddelande');
            ...
            //Page Verifiera/Kontrollera Avtal  --Verify/Check Agreement
            await this.page.getByRole('heading', { name: 'Verifiera/Kontrollera Avtal' }).click();
            await this.page.getByText('Detta avtal kommer att omförhandla, vänligen verifiera och klicka "next"').click();
            await this.page.getByText('Vänligen notera att detta avtal har växel lösning och dess priser får inte omförhandlas (priser måste vara samma som i TUPP om inkluderat i avtalet) Vänligen notera att detta avtal har en längd av').click();
            await this.page.getByRole('button', { name: 'Nästa' }).click();
            ...
            /*}if (TestData.get("LegacyNegotiation") === "LegacyOmforhandlingOE"){
                ...
                await this.cartFrame.locator("//button[contains(text(),'Validera priser') or contains(text(),'Validera varukorgen') or contains(text(), 'RepriceCart ')]").click();   
                await this.page.waitForTimeout(5000);     
                await this.cartFrame.locator("//button[contains(text(),'Slutför konfiguration') or contains(text(),'SubmitOfferForApproval')]").click();
                await this.page.waitForTimeout(5000);
            */    

            }else {
                // Page Skicka mandatförfrågan?
                await this.page.getByRole('heading', { name: 'Skicka mandatförfrågan?' }).click();
                await this.page.getByLabel('KOMMENTARER').click();
                await this.page.getByLabel('KOMMENTARER').fill('Test Automation');
                await this.page.getByRole('button', { name: 'Nästa' }).click();
            ...
            }else if ((KanOmforhandlasInMADAM.trim() ==='Y' && ContractInSalesforceAvtal.trim() ==='Y')){
            console.log("SalesFlowType == LegacyNegotiation :: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + " And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + " '.");                          
            ...
            console.log("Condition not Handled for SalesFlowType == LegacyNegotiation :: KanOmforhandlas In MADAM = '" + KanOmforhandlasInMADAM + " And Contract In Amanda SalesforceAvtal = '" + ContractInSalesforceAvtal + " '.");                          
            ...
            */
            ...
            console.log ("SalesFlowType = Inforhandling");
            ...
            await this.page.getByText('Införhandling : Komplettering av abonnemangsformer som ej fanns med vid avtalstecknande.', { exact: true }).click();
            await this.page.getByRole('button', { name: 'Nästa' }).click();
        } else if (TestData.get("SalesFlowType") === "Tillaggsforhandling") {

            console.log ("SalesFlowType = Tillaggsforhandling");
            ...
            await this.page.getByText('Omförhandling : Aktivt avtal som skall omförhandlas med möjlighet att lägga till nya produkter/tjänster.', { exact: true }).click();
            await this.page.getByText('Tilläggsförhandling : Enbart tillägg av nya produkter/tjänster i aktivt avtal. (Det går ej att omförhandla befintliga produkter/tjänster).', { exact: true }).click();
            await this.page.getByRole('button', { name: 'Nästa' }).click();
        }
    }
    ...
    async IntaktseffektkalkylCreation() {
        ...
        await this.page.locator("//button[@name='Opportunity.Revenue_Impact_Calculation']").first().click();
        await this.page.locator("//div[contains(@class,'active') and contains(@class,'window')]//button[@name='controllerFld' and @aria-label='Category']").first().click();        
    ...
    async performEsigningOnOpportunity(TestData, utilityFunction, OpportunityID) {
        ...
        await this.page.goto(secretsData.get("environmentURL") + "/lightning/r/Opportunity/" + OpportunityID + "/view");
        if (TestData.get("OrgType") === "SOHO") {
            await this.page.locator("//div[contains(@class,'active') and contains(@class,'window')]//button[text()='Skicka dokument för e-signering']").hover();
        ...
        if ("OpportunityAvtalEsigning" === TestData.get("SalesFlowType")) {
            ...
            await this.cartFrameScrive.getByRole('cell', { name: 'Test Notering för arkivering', exact: true }).click();
            await this.cartFrameScrive.locator("//table[@class='detailList']//td[text()='Status']//following-sibling::td[text()='Draft']").click();
            await this.cartFrameScrive.locator("//table[@class='detailList']//td[text()='Leveranssätt']//following-sibling::td[text()='Email']").click();
