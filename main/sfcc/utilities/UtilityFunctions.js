...
class UtilityFunctions {
    ...
    async SetTERESProductsAndSubProducts(AMANDATestData) {
        ...
        if ("Large Organisation" === recordType) {
            ...
            AMANDATestData.set("SalesRepUser", "Small Sales Rep");
        } else if ("Organization" === recordType) { 
            AMANDATestData.set("OrgType", "SOHO");
            //AMANDATestData.set("SalesRepUser", "Soho Ds SalesRep");
            AMANDATestData.set("SalesRepUser", "Medium & Large SalesRep");
        }
    ...
    async processTERESTestDataManual(AMANDATestData) {
        if (process.env.ORGNUMBER) {
            AMANDATestData.set("OrgNumber", "5599914792"); //5599916268
            AMANDATestData.set("Products", "Telia Bredband Start");
            //AMANDATestData.set("SubProducts", "Jobbmobil 5 GB;Jobbmobil 10 GB;Bredband Start 100 Mbit/s;");
            AMANDATestData.set("SubProducts", "Bredband Start 10 Mbit/s;Bredband Start 50 Mbit/s;Bredband Start 500 Mbit/s; Bredband Start 1 Gbit/s;Engångsavgift för anslutningar 24 Mån;");            
        }
    }


    async SetTERESProductsAndSubProductsManual(AMANDATestData) {
        //await this.processTERESTestData(AMANDATestData);
        await this.processTERESTestDataManual(AMANDATestData);
        var recordTypeID = await this.RunSOQLQuery("select RecordTypeId from Account where Org_Nr__c = '" + AMANDATestData.get("OrgNumber") + "'");
        var recordType = await this.RunSOQLQuery("select Name from RecordType where id = '" + recordTypeID + "'");
        if ("Large Organisation" === recordType) {
            AMANDATestData.set("OrgType", "LARGE");
            //AMANDATestData.set("SalesRepUser", "RA Sales Rep");
            AMANDATestData.set("SalesRepUser", "Small Sales Rep");
        //} else if ("SME/SOHO Organisation" === recordType) { --B2XSET-281864) Planned existing Account Record Type Label changed to Organization          
        } else if ("Organization" === recordType) {
            AMANDATestData.set("OrgType", "SOHO");
            //AMANDATestData.set("SalesRepUser", "Soho Ds SalesRep");
            AMANDATestData.set("SalesRepUser", "Medium & Large SalesRep");
        ...
        var Product = AMANDATestData.get("Products");
        //create  generic for Subproduct check - var SubProduct = "[" + AMANDATestData.get("MobilupplaggTotalSubProducts")+ "]" + "[" + AMANDATestData.get("BredbandStartSubProducts")+"]";
        /*
        for (let i = 0; i < Product.length; i++){
            var Subproducts =             
            var SubProduct =  "[" + AMANDATestData.get("BredbandStartSubProducts")+"]";
        }
        */
        var SubProduct =  "[" + AMANDATestData.get("BredbandStartSubProducts")+"]";
        console.log ("TERES Products are ::==  "+ Product); 
        console.log ("TERES SubProducts are ::==  " + SubProduct);
        var Product = (AMANDATestData.get("Products")).split(";");    
        var trimmedSubProducts = SubProduct.slice(1, -1);    
        //var trimmedSubProducts = (AMANDATestData.get("SubProducts")).slice(1, -1);     
        var SubProducts = (trimmedSubProducts).split("][");            
        for (let i = 0; i < Product.length; i++) {
            switch (Product[i]) {
                ...
                case "Cloud VPN SD-Wan/Firewall":
                    ...
                    case "Edge Security":
                    {
                        AMANDATestData.set("EdgeSecuritySubProducts", SubProducts[i]);
                        break;
                    }
                case "Datanet":
                    {
    ...
    async ReadDataFromExcel() {
        ...
        var TestDataPath = "./resources/sfcc-test-data/TestDataSheet_";
        var Sheet;
        switch (secretsData.get("environment")) {
            ...
            case "ST2Environment":
                ...
                break;
            case "SITEnvironment":
                TestDataPath = TestDataPath + "SITEnvironment.xlsx";
                break;
        ...
        if (testName.includes('SalesEnabler')) {
            Sheet = 'SalesEnabler';
        }
        ...
        if (testName.includes('AmandaOTA')) {
            Sheet = 'AmandaOTA';
        }
        if (testName.includes('Standalone')) {
            Sheet = 'StandaloneCases';
        }
            ...
            worksheet.eachRow({ includeEmpty: true }, (row, rowNumber) => {
                ...
                if (currentTestCase === testName) {
                    ...
                    row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
                        ...
                        console.log ("testData ReadDataFromExcel  :: "+ headers[colNumber - 1] + "  - - - " + cell.value);
                    });
                }
            });   
    ...
    async ReadDataFromExcelForDataMigration(iteration) {
        ...
        try {
            ...
            headerRow.eachCell({ includeEmpty: true }, (cell) => {
                headers.push(cell.value);
                //console.log( " headers ARRAY cell.value  is " + cell.value + ", cell is :: "+ cell);
            });
            const testData = new Map();
            worksheet.eachRow({ includeEmpty: true }, (row, rowNumber) => {
                const currentTestCase = row.getCell(1).value; // Assuming test case name is in the first column (column A)
                //console.log("currentTestCase Name from testdata excel is  ::"+ currentTestCase + ", And Test case name is ::" + testName);
                if (currentTestCase === testName) {
                    row.eachCell({ includeEmpty: true }, (cell, colNumber) => {
                        testData.set(headers[colNumber - 1], cell.value);
                        //console.log( "testdata Map read  rowNumber - "+ rowNumber+ "row :: "+ row + " ,And colNumber is - " + (colNumber - 1) );
                    });
                }
            });
            return testData;
    ...
    async WriteDataToExcel(columnName, dataToWrite) {
        const secretsData = await this.fetchEnvironmentCreds();
        var TestDataPath = "./resources/PerformanceTestResults/PerformanceTestResults_";
        var Sheet;
        switch (secretsData.get("environment")) {
            case "TrainingEnvironment":
                TestDataPath = TestDataPath + "TrainingEnvironment.xlsx";
    ...
    async RunSOQLQuery(Query) {
        ...
        console.log ("RUN SQL Query :: " +Query);
        ...
        if (RecordNotNull) {
            ...
            //console.log (" RunSOQLQuery Return Value :: " +secondValue);          
        }
        return secondValue;
    }
    ...
    async TodaysDateWithMonthDot() {
        const today = new Date();
        const yyyy = today.getFullYear();
        let mm = today.getMonth() + 1;
        const mmm = monthNames[mm];
        let dd = today.getDate();
        if (dd < 10) dd = '0' + dd;
        //if (mm < 10) mm = '0' + mm;
        console.log("Todays Date With Month Dot :: " + yyyy + '-' + mmm +'.'+'-' + dd);
        return yyyy + '-' + mmm +'.'+'-' + dd;
    }

    async TodaysDateWithMottagetDatumformat() {

        //*Mottaget Datum swidish
        const monthNames = ["januari", "februari", "mars", "april", "maj", "juni", "juli", "augusti", "september", "oktober", "november", "december"];
        const today = new Date();
        const yyyy = today.getFullYear();
        let mm = today.getMonth() + 1;
        const mmm = monthNames[mm];
        //9 jan. 2025, 21 feb. 2025, 28 mars 2025, 26 apr. 2025, 31 maj 2025, 5 juni 2026, 27 juli 2025, 27 aug. 2025, 25 sep. 2025, 25 okt. 2025, 21 nov. 2025, 27 dec. 2025
        if(mm === '3' ||mm === '6'||mm === '7' ){
            const mmmm = monthNames[mm].substring(0, 4)
            var mottagetmmmm = mmmm;
         } else if (mm === '5'){
            const mmm = monthNames[mm].substring(0, 3)
            var mottagetmmmm = mmm;
         } else {
            const mmm = monthNames[mm].substring(0, 3)
            var mottagetmmmm = mmm +'.';            
         }        
        ...
        if (dd < 10) dd = '0' + dd;
        //if (mm < 10) mm = '0' + mm;
        console.log("Todays Date With Month Dot :: " + dd + ' ' + mottagetmmmm +' ' + yyyy);
        //return yyyy + '-' + mmm +'.'+'-' + dd;
        return dd + ' ' + mottagetmmmm +' ' + yyyy;
    }

    async GetFutureWeekday(daysToAdd) {
        const today = new Date();
        let addedDays = 0;
    ...
    async getFutureWeekdayDate(daysToAdd) {
        const today = new Date();
        let addedDays = 0;
    ...
    async RunUpdateDML(ObjectAPIName, RecordId, updatedData) {
        ...
        console.log (" function RunUpdateDML " + " RecordId :: " + RecordId );
        const secretsData = await this.fetchEnvironmentCreds();
        const username = secretsData.get("username");
        const password = secretsData.get("password");
        const clientId = secretsData.get("clientId");
        ...
        const { access_token, instance_url, } = await response.json();        
        console.log("  RunUpdateDML :: RecordId is  ::  " + RecordId);        
    ...
    async deleteRecordFromOrg(object, recordId) {
        ...
        //console.log ("DeleteRecordFromOrg SQL recordId :: " +recordId);
    ...
    async OrderDetailsFetch(OrgNumber){
         var OrgID = await this.RunSOQLQuery("SELECT Id FROM Account where Org_Nr__c ='" + OrgNumber + "'");
         var OrderNumber = await this.RunSOQLQuery("SELECT OrderNumber from Order where AccountId = '" + OrgID + "' ORDER BY CreatedDate DESC LIMIT 1");
         console.log("************************* Order Number *****************************");
         console.log("************************* "+ OrderNumber +" *****************************");
         return OrderNumber
    }

    async OrderDetailsFetchAndDelete(OrgNumber){
         var OrgID = await this.RunSOQLQuery("SELECT Id FROM Account where Org_Nr__c ='" + OrgNumber + "'");
         var OrderNumber = await this.RunSOQLQuery("SELECT OrderNumber from Order where AccountId = '" + OrgID + "' ORDER BY CreatedDate DESC LIMIT 1");
         var OrderId = await this.RunSOQLQuery("SELECT id from Order where AccountId = '" + OrgID + "' ORDER BY CreatedDate DESC LIMIT 1");

         console.log("************************* Order Number *****************************");
         console.log("************************* "+ OrderNumber +" *****************************");
         console.log("************************* Order Id *****************************");
         console.log("************************* "+ OrderId +" *****************************");
         this.deleteRecordFromOrg("Order", OrderId)
    }


    async customScreenShot(page,pageName){
       //await page.screenshot({path:'test-results/Screenshots/'+Date.now()+'_'+this.TestCaseName+'_'+pageName+'.png',fullPage:true})
    }
