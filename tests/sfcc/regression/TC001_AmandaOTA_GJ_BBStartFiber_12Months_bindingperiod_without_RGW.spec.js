...
const TestCaseName = 'TC001_AmandaOTA_GJ_BBStartFiber_12Months_bindingperiod_without_RGW';

test('TC001_AmandaOTA_GJ_BBStartFiber_12Months_bindingperiod_without_RGW.', async function ({ browser }) {
  
  //Setting up first browser page
  /*const context = await browser.newContext({
    recordVideo: {
      dir: 'videos/',   
      size: { width: 1280, height: 720 } // HD video size
    }
  });*/
  const context = await browser.newContext();
  ...
  try{
   ...
   // await loginPage.adminUserLoginForStandAlone(utilityFunctionLocal,"Industries CPQ");
    ...
    /*const videoPath = page.video()?.path(); // this returns a promise
    const savedVideoPath = await videoPath;
